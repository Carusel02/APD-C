...
void *sample_grid_thread(void *arg) {
    ...
    printf("contour pointer func 1 = %p\n", info_thread->contour_map);
   ...
   info_thread->grid[p][q] = 0;
    ...
    p = (*info_thread->image)->x / info_thread->step_x;
    q = (*info_thread->image)->y / info_thread->step_y;

    start = thread_id * (double)p / NUMBER_THREADS;
    end = min((thread_id + 1) * (double)p / NUMBER_THREADS, p);

    ppm_image **contour_map = info_thread->contour_map;
    printf("contour pointer func = %p\n", contour_map);

    for (int i = start; i < end; i++) {
        for (int j = 0; j < q; j++) {
            unsigned char k = 8 * info_thread->grid[i][j] + 4 * info_thread->grid[i][j + 1] + 2 * info_thread->grid[i + 1][j + 1] + 1 * info_thread->grid[i + 1][j];
            update_image(*info_thread->image, contour_map[k], i * STEP, j * STEP);
        }
    }
